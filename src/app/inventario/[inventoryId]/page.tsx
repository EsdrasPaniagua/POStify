...
import { Package, Search, Plus, Trash2, Edit2, DollarSign, ShoppingCart, Tag, X, ScanLine, Image as ImageIcon, Warehouse, ArrowLeft } from 'lucide-react';
...
export default function InventarioPage() {
  ...
  const inventoryId = params?.inventoryId as string | undefined;
  const isGeneral = inventoryId === 'general';
 ...
 const loadVariants = async () => {
  ...
  const filteredProducts = products.filter(p => {
    ...
    // Filtro por inventario según la ruta
    const matchInventory = !inventoryId
      ? true
      : isGeneral
        ? !p.inventoryId
        : p.inventoryId === inventoryId;
    return matchCat && matchLowStock && matchSearch && matchVariants && matchInventory;
  });
...
const resetForm = () => {
  ...
  setSelectedInventoryId(product.inventoryId || '');
  setIsDialogOpen(true);
};
...
groupedProducts.sort((a, b) => {
  ...
  return (
    <div className="p-4 sm:p-6">
      {/* Banner inventario activo */}
      {inventoryId && (
        <div
          className="flex items-center gap-3 mb-4 px-4 py-2.5 rounded-xl text-white text-sm font-medium"
          style={{ backgroundColor: isGeneral ? '#6b7280' : (currentInventory?.color || '#6b7280') }}
        >
          <button onClick={() => router.push('/inventario')} className="hover:opacity-70 transition-opacity">
            <ArrowLeft className="h-4 w-4" />
          </button>
          <Warehouse className="h-4 w-4" />
          <span>{isGeneral ? 'General — Sin inventario asignado' : currentInventory?.name || 'Inventario'}</span>
        </div>
      )}
            ...
            {idx === 0 && (
              <div className="flex flex-col gap-1">
                ...
                {p.inventoryId && (() => {
                  ...
                  return inv ? (
                    <span
                      className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white w-fit"
    ...
    <div className="space-y-4">
      ...
      {inventories.length > 0 && (
        <div>
          ...
          >
            <option value="">Sin inventario específico</option>
            {inventories.map(inv => (
